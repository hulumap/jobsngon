...
export class Template2Component implements OnInit {
  ...
  saveResume() { // hàm lưu thêm dữ liệu màu sắc, ngôn ngữ 
    this.disable = true
    this.jobsngon.saveResume(this.resume, 2).then((res) => {
      if (res) this.disable = false
    }, err => console.log(err))
  }

  changeLang(lang) { // hàm lấy ngôn ngữ cho CV, đồng thời thay đổi ngôn ngữ cho cv
    this.jobsngon.getLocalCV().then((data) => {
      if (data) this.resume = this.jobsngon.changeLang(lang, this.resume, true)
      else this.resume = this.jobsngon.changeLang(lang, this.resume, false)
    })
  }
  ...
  viewOnline() { // hàm xem cv online
    this.jobsngon.viewOnline(this.resume, '2')
  }
  ...
  exportPDF() { // hàm in CV, hoặc tải về pdf
    this.down = true
    let data = document.getElementById('tmp2');

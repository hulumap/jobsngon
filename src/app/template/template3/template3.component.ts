...
export class Template3Component implements OnInit {
  ...
  saveResume() { // hàm lưu thêm dữ liệu màu sắc, ngôn ngữ 
    this.disable = true
    this.jobsngon.saveResume(this.resume, 3).then((res) => {
  ...
  viewOnline() { // hàm xem cv online
    this.jobsngon.viewOnline(this.resume, '3')
  }
  ...
  exportPDF() { // hàm in CV, hoặc tải về pdf
    this.down = true
    let data = document.getElementById('tmp3');
  ...
  dropS(event: CdkDragDrop<string[]>) {
    moveItemInArray(this.resume.dataS, event.previousIndex, event.currentIndex);
  }
